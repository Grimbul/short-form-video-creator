...
class Parser:
    ...
    def save_posts(self, post_dictionary, save_file):
        ...
        with open(save_file, 'w', encoding='utf-8') as writer:
            for post_num, title in enumerate(post_dictionary):
                body = post_dictionary[title]

                writer.write(f"Post number: {post_num}\n")
                writer.write(title + "\n")
                writer.write(body + "\n\n")

    def read_post(self, post_filename, post_number):
        """
        Gives post corresponding to given number. Index begins at 0
        :param post_filename: file to read posts from
        :param post_number: number of post desired to be read
        :return: post at specified number
        """
